...
# 미로 경로 탐색
  대충 Graph 그리고 이걸 dict로 바꾸는 과정 꼭 해볼것. 대충 미로를 보고서 완성하는 그거임..

# BFS // 너비 우선 탐색. Qu 사용함 + (최단거리를 보장해줌) 
  #미로 찾기 프로그램(그래프 탐색)
...
여기서 미로에 변칙을 준다(구멍을 낸다)
라면 해당 maze값에 뚫린 벽 간 노드를 이어주면 된다.
...
print(solve_maze(maze, f'{start}input the Start Point, input the End Point{end}'))
